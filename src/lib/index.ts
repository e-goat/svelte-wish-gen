...
export function generateShareCode(): void {
    ...
    if (currentCard.audioBlob) {
        ...
        reader.onload = function(): void {
            ...
            const shareCode: string = btoa(JSON.stringify(cardData));
...
function getGradientStops(templateName: TemplateNames): string {
    const gradients: Record<TemplateNames, string> = {
        birthday: '<stop offset="0%" stop-color="#ff9a9e"/><stop offset="100%" stop-color="#fecfef"/>',
        love: '<stop offset="0%" stop-color="#ffecd2"/><stop offset="100%" stop-color="#fcb69f"/>',
        celebration: '<stop offset="0%" stop-color="#a8edea"/><stop offset="100%" stop-color="#fed6e3"/>'
    };
    return gradients[templateName] || gradients.birthday;
}

function createMessageLines(message: string, x: number, startY: number): string {
    ...
    for (let i = 0; i < words.length; i++) {
        const testLine: string = currentLine + (currentLine ? ' ' : '') + words[i];
...
export function importCard(): void {
    ...
    try {
        const cardData: CardData = JSON.parse(atob(code));
        
        // Validate imported data
        if (!cardData.template || !templates[cardData.template as TemplateNames]) {
        ...
        if (previewCard) {
            previewCard.classList.remove('hidden');
            const actionButtons = previewCard.querySelector('.action-buttons') as HTMLElement;
            if (actionButtons) {
        ...
        alert('Invalid card code. Please check and try again.');
    }
}
